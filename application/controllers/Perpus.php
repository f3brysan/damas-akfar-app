...
class Perpus extends CI_Controller {
	public function __construct() {
		...
		if ($this->session->userdata('level') !== "perpus") {
	...
	public function index() {
		...
		// dump($data);
		$this->load->view('perpus/navbar', $data);
		$this->load->view('perpus/index');
	...
	public function data_kti_yudisium() {
		...
		// dump($data);
		$this->load->view('perpus/navbar', $data);
		$this->load->view('perpus/yudisium/index');
		$this->load->view('perpus/footer');
	}
	...
	public function waiting_list_kti() {
		...
		// $data['count'] = $this->m_admin->countnilaiterisi($last);
		// $data['count'] = $this->m_dosen->countdosennilai($nidn);
		$data['unapprove_list'] = $this->m_dosen->get_data_kti_perpus_unapprove($kode);
		$data['approve_list'] = $this->m_dosen->get_data_kti_perpus_approve($kode);
		// dump($data);
		$this->load->view('perpus/navbar', $data);
		$this->load->view('perpus/yudisium/data_list');
		$this->load->view('perpus/footer');
	}
	...
	public function unapprove_mhs_kti() {		
		...
		$where = array('nim' => $nim);
		$this->m_dosen->hapus_data($where, 'tb_bebas_perpus');		
		 redirect("perpus/waiting_list_kti/$kode");
	}
	...
	public function approve_mhs_kti()
    {
        ...
        // dump($create);
        $this->m_admin->inputakun($create, 'tb_bebas_perpus');
        redirect("perpus/waiting_list_kti/$kode");
    }
